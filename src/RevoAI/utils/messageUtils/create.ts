import type { FileType } from '@/RevoAI/types'
...
export function createCodeBlock(
  ...
  language: string,
  overrides: Partial<
    Omit<CodeMessageBlock, 'id' | 'messageId' | 'type' | 'content' | 'language'>
  > = {},
): CodeMessageBlock {
...
export function createTranslationBlock(
  ...
  overrides: Partial<
    Omit<TranslationMessageBlock, 'id' | 'messageId' | 'type' | 'content' | 'targetLanguage'>
  > = {},
): TranslationMessageBlock {
...
export function createCitationBlock(
  ...
  overrides: Partial<
    Omit<CitationMessageBlock, 'id' | 'messageId' | 'type' | keyof typeof citationData>
  > = {},
): CitationMessageBlock {
  const { response, ...baseOverrides } = {
    ...citationData,
    ...overrides,
  }
...
export function createMessage(
  ...
  sessionId: string,
  overrides: PartialBy<
    Omit<Message, 'role' | 'sessionId' | 'createdAt' | 'status'>,
    'blocks' | 'id'
  > = {},
): Message {
  const now = new Date().toISOString()
...
export function createAssistantMessage(
  sessionId: string,
  overrides: Partial<
    Omit<Message, 'id' | 'role' | 'sessionId' | 'createdAt' | 'type' | 'status'>
  > = {},
): Message {
  const now = new Date().toISOString()
...
export const resetAssistantMessage = (
  ...
  return {
    ...
    modelDisplayName: originalMessage.modelDisplayName, // 保留原始模型展示名称
