...
import type { Message } from '@/RevoAI/types/newMessage' // Assuming correct Message type import
import { MessageBlockType } from '@/RevoAI/types/newMessage'
  ...
  return messages
    ...
    .filter((message) => {
      const mainTextBlock = message.blocks
        ?.map((blockId) => messageBlocksStore.selectById(blockId))
