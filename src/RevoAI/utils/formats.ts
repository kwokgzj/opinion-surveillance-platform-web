 ...
 * @returns 图片URL数组
 */
export function extractImageUrls(message: Message): (string | undefined)[] {
  const imageBlocks = findImageBlocks(message)
  return imageBlocks.map((block) => block.url)
}

/**
 * 处理 Mermaid 代码块中的特殊字符转义，防止被 Markdown 解析器误识别为列表
 * @param text 要处理的文本
 * @returns 处理后的文本
 */
export function escapeMermaidDots(text: string): string {
  // 匹配 mermaid 代码块
  const mermaidBlockRegex = /(```mermaid\s*\n)([\s\S]*?)(```)/g

  return text.replace(mermaidBlockRegex, (match, start, content, end) => {
    // 在 mermaid 代码块内容中，将 "数字." 模式的点号进行转义
    // 避免被 Markdown 解析器识别为编号列表
...
export function addImageFileToContents(messages: Message[]) {
  ...
  return messages.map((message) =>
    message.id === lastAssistantMessage.id ? updatedAssistantMessage : message,
  )
}
