import type { ExternalToolResult, MCPToolResponse, WebSearchResponse } from '@/RevoAI/types'
...
import { AssistantMessageStatus } from '@/RevoAI/types/newMessage'
...
export function createStreamProcessor(callbacks: StreamProcessorCallbacks = {}) {
  ...
  return (chunk: Chunk) => {
    try {
      ...
      if (
        data.type === ChunkType.MCP_TOOL_COMPLETE &&
      ...
      if (
        data.type === ChunkType.LLM_WEB_SEARCH_IN_PROGRESS &&
