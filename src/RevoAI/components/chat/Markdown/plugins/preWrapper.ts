...
export function preWrapperPlugin(md: MarkdownIt, options: Options) {
  ...
  md.renderer.rules.fence = (...args) => {
    ...
    const copyButtonTitle = options.codeCopyButtonTitle || "复制代码";

    return `
      <div class="markdown-code-wrapper flex language-${lang}${getAdaptiveThemeMarker(
      options
    )}${active}">
        <div class="markdown-code-header">
          <span class="markdown-code-lang">${getBaseLanguageName(lang)}</span>
          <button class="markdown-code-copy">
            <span class="markdown-copy-text default">${copyButtonTitle}</span>
