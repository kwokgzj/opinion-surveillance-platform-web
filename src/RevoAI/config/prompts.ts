...
export const FLOW_CHAT_PROMPT = `Unless the user explicitly states that a flowchart needs to be output, no flowchart is output. If the user does not specify a flowchart format, the mermaid format is used by default.`

export const FLOW_CHAT_PROMPT_ZH = `除非用户明确要求输出流程图，否则不输出流程图。如果用户没有指定流程图格式，默认使用mermaid格式。`

export const getFlowChatPrompt = (language: string) => {
  return language.startsWith('zh') ? FLOW_CHAT_PROMPT_ZH : FLOW_CHAT_PROMPT
}
