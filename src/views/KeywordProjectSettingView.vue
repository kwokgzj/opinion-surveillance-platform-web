          ...
          <div class="platform-multiselect">
            <div class="multiselect-container" @click="toggleDropdown">
              <div class="selected-platforms">
                <span v-if="selectedPlatforms.length === 0" class="placeholder">请选择抓取平台</span>
...
export default {
  ...
  methods: {
    ...
    this.selectedPlatforms = ['Youtube', 'Twitter', 'Instagram', 'Facebook']; // 全选平台
    ...
    loadProjectData(projectId) {
      ...
      this.selectedPlatforms = ['Youtube', 'Twitter'];
      this.keywords = [{ word: '测试关键字', include: '', exclude: '' }];
      this.excludeLinks = ['fsdddddddd','fskdfjhskdjf','hvoxcjuo'];
    },
    ...
    saveInitialFormData() {
      this.initialFormData = {
        ...
        excludeLinks: [...this.excludeLinks]
      };
    },
    ...
    cancelChanges() {
      if (!this.isEditMode && this.projectStatus === 'creating') {
        ...
        if (this.initialFormData) {
          ...
          this.selectedPlatforms = [...this.initialFormData.selectedPlatforms];
          this.keywords = JSON.parse(JSON.stringify(this.initialFormData.keywords));
          this.excludeLinks = [...this.initialFormData.excludeLinks];
          this.excludeLinksText = this.excludeLinks.join('\n');
    ...
    togglePlatform(platformValue) {
      const index = this.selectedPlatforms.indexOf(platformValue);
      if (index > -1) {
        this.selectedPlatforms.splice(index, 1);
      } else {
        this.selectedPlatforms.push(platformValue);
      }
    },
    ...
    handleExcludeLinksChange() {
      this.excludeLinks = this.excludeLinksText
        .split('\n')
        .map(line => line.trim())
