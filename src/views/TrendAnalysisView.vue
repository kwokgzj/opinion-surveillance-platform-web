...
const processTrendAnalysisData = (data: TrendAnalysisData, retryCount = 0) => {
  ...
  if (
    (!videoTotalChart ||
      !videoIncrementalChart ||
      !postTotalChart ||
      !postIncrementalChart ||
      !mentionTimeChart ||
      !mentionMediaChart ||
      !mentionLanguageChart ||
      !mentionRegionChart) &&
    retryCount < 3
  ) {
...
const initAllCharts = () => {
  ...
  nextTick(() => {
    setTimeout(() => {
      initChart('videoTotalChart', (chart) => {
        videoTotalChart = chart
      })
      initChart('videoIncrementalChart', (chart) => {
        videoIncrementalChart = chart
      })
      initChart('postTotalChart', (chart) => {
        postTotalChart = chart
      })
      initChart('postIncrementalChart', (chart) => {
        postIncrementalChart = chart
      })
      initChart('mentionTimeChart', (chart) => {
        mentionTimeChart = chart
      })
      initChart('mentionMediaChart', (chart) => {
        mentionMediaChart = chart
      })
      initChart('mentionLanguageChart', (chart) => {
        mentionLanguageChart = chart
      })
      initChart('mentionRegionChart', (chart) => {
        mentionRegionChart = chart
      })
      ...
      areaStyle: {
        color: {
          ...
          colorStops: [
            {
              offset: 0,
              color: s.color + '4D', // 30% opacity
            },
            {
              offset: 1,
              color: s.color + '1A', // 10% opacity
            },
          ],
        },
      },
...
/* WebKit 浏览器原生滚动条样式 */
.trend-analysis-page::-webkit-scrollbar {
  width: 14px;
  background-color: transparent;
}

.trend-analysis-page::-webkit-scrollbar-track {
  background: #f5f5f5;
  border-radius: 7px;
  margin: 4px 0;
}

.trend-analysis-page::-webkit-scrollbar-thumb {
  background: #c0c4cc;
  border-radius: 7px;
...
.trend-analysis-page::-webkit-scrollbar-thumb:hover {
  background: #a8a8a8;
}

.trend-analysis-page::-webkit-scrollbar-thumb:active {
  background: #909399;
}
